...
class OrderInfo:
    ...
    def update(self, order_info: "OrderInfo"):
...
class Exchange(metaclass=abc.ABCMeta):
    """Exchange operations used by :class:`PositionManager`."""
    ...
    async def create_market_order(
        ...
        """Create a market order and return its id."""
        raise NotImplementedError()
...
class PositionManager:
    ...
    def __init__(
            self, exchange: Exchange, position_amount: Decimal, quote_symbol: str,
            stop_loss_pct: Decimal, borrowing_disabled: bool = True
    ):
    ...
    async def get_position_info(self, pair: bs.Pair) -> Optional[PositionInfo]:
        async with self._pos_mutex[pair]:
            ...
            if pos_info and pos_info.order_open:
                order_info = await self._exchange.get_order_info(pair, pos_info.order.id)
                pos_info.order.update(order_info)
    ...
    async def switch_position(self, pair: bs.Pair, target_position: bs.Position, force: bool = False):
        ...
        async with self._pos_mutex[pair]:
            ...
            if current_pos_info and current_pos_info.order_open:
                ...
                await self._exchange.cancel_order(pair, current_pos_info.order.id)
                order_info = await self._exchange.get_order_info(pair, current_pos_info.order.id)
                current_pos_info.order.update(order_info)
            ...
            order_id = await self._exchange.create_market_order(operation, pair, order_size)
            logger.info(StructuredMessage("Order created", id=order_id))
            order = await self._exchange.get_order_info(pair, order_id)
    ...
    async def on_trading_signal(self, trading_signal: bs.TradingSignal):
        ...
        try:
            ...
            for pair, target_position in pairs:
                # Map short signals to neutral when borrowing is disabled (e.g. spot accounts).
    ...
    async def on_order_update(
            self, pair: bs.Pair, order_id: str, is_open: bool, amount: Decimal, amount_filled: Decimal,
            fill_price: Optional[Decimal]
    ):
        logger.info(StructuredMessage(
            "Order updated", id=order_id, pair=pair, is_open=is_open, amount=amount,
            amount_filled=amount_filled, avg_fill_price=fill_price
        ))

        # Update the position info.
        async with self._pos_mutex[pair]:
            pos_info = self._positions[pair]
            if order_id == pos_info.order.id and amount_filled >= pos_info.order.amount_filled:
                pos_info.order.is_open = is_open
                pos_info.order.amount_filled = amount_filled
                pos_info.order.fill_price = fill_price
