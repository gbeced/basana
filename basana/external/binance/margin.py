...
class Account(metaclass=abc.ABCMeta):
    ...
    async def create_stop_limit_order(
        ...
        **kwargs: Dict[str, Any],
        ...
        return await self.create_order(
            margin_requests.StopLimitOrder(
                operation,
                pair,
                amount,
                stop_price,
    ...
    async def get_order_info(
        ...
        if include_trades:
            trades = [
                Trade(trade)
                for trade in await self.client.get_trades(order_book_symbol, order_id=order_info["orderId"])
            ]
    ...
    async def get_open_orders(self, pair: Optional[Pair] = None) -> List[OpenOrder]:
        ...
        if pair:
            order_book_symbol = helpers.pair_to_symbol(pair)
        return [OpenOrder(open_order) for open_order in await self.client.get_open_orders(order_book_symbol)]
