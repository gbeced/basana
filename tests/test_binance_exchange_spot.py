...
def test_account_balances(binance_http_api_mock, binance_exchange):
    binance_http_api_mock.get(
        re.compile(r"http://binance.mock/api/v3/account\\?.*"),
        status=200,
        payload={
            "makerCommission": 10,
            "takerCommission": 10,
            ...
            "accountType": "SPOT",
            "balances": [
                {"asset": "BTC", "free": "0.00000053", "locked": "1.00000000"},
...
def test_order_info(
    ...
    binance_http_api_mock.get(re.compile(r"http://binance.mock/api/v3/order\\?.*"), status=200, payload=order_payload)
    binance_http_api_mock.get(
...
def test_error_retrieving_order_info(binance_http_api_mock, binance_exchange):
    binance_http_api_mock.get(re.compile(r"http://binance.mock/api/v3/order\\?.*"), status=500, payload={})
    ...
    [
        (
            lambda e: e.spot_account.create_oco_order(
                ...
                Decimal("23000"),
                Decimal("10000"),
                stop_limit_price=Decimal("10000"),
            ),
            {
                "orderListId": 77826706,
                "contingencyType": "OCO",
