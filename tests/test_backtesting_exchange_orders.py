...
@pytest.mark.parametrize("order_plan, immediate_order_processing", [
    ...
    (
        {
            ...
            datetime.date(2000, 12, 29): [
                # # Not enough balance to fill the order.
                # (
                #     lambda e: e.create_market_order(OrderOperation.BUY, aapl_p, Decimal("26")),
                #     [],
                #     [
                #         dict(
                #             is_open=False,
...
def test_order_requests(order_plan, immediate_order_processing, backtesting_dispatcher):
    ...
    async def impl():
        # ORCL bars.
        e.add_bar_source(bars.CSVBarSource(orcl_p, abs_data_path("orcl-2000-yahoo-sorted.csv")))
        ...
        e.subscribe_to_bar_events(orcl_p, on_bar)
        e.subscribe_to_bar_events(aapl_p, on_bar)
        e.subscribe_to_order_events(on_order_updated)
...
@pytest.mark.parametrize("order_request", [
    requests.MarketOrder(OrderOperation.SELL, orcl_p, Decimal(1)),
    requests.LimitOrder(OrderOperation.BUY, orcl_p, Decimal(1000), Decimal("1")),
    requests.LimitOrder(OrderOperation.SELL, orcl_p, Decimal(1), Decimal("1")),
    requests.StopOrder(OrderOperation.BUY, orcl_p, Decimal(1000), Decimal("1")),
    requests.StopOrder(OrderOperation.SELL, orcl_p, Decimal(1), Decimal("1")),
    requests.StopLimitOrder(
        OrderOperation.BUY, orcl_p, Decimal(1000), Decimal("1"), Decimal("1")
    ),
    requests.StopLimitOrder(
        OrderOperation.SELL, orcl_p, Decimal(1), Decimal("1"), Decimal("1")
    ),
])
...
def test_balance_is_on_hold_while_order_is_open(backtesting_dispatcher):
    async def impl():
        ...
        created_order_1 = await e.create_order(
            requests.LimitOrder(OrderOperation.BUY, orcl_p, Decimal(1), Decimal(750))
        )
        ...
        created_order_2 = await e.create_order(requests.LimitOrder(
            OrderOperation.BUY, orcl_p, Decimal(1), Decimal(200)
        ))
        assert (await e.get_balance("ORCL")).available == Decimal(0)
        ...
        with pytest.raises(exchange.Error):
            await e.create_order(requests.LimitOrder(
                OrderOperation.BUY, orcl_p, Decimal("1"), Decimal(760)
            ))
