...
async def test_create_get_and_cancel_order(backtesting_dispatcher):
    e = exchange.Exchange(
...
async def test_order_updates_have_priority(backtesting_dispatcher):
    e = exchange.Exchange(
        backtesting_dispatcher,
        {"USD": Decimal("50000")}
    )
...
async def test_order_info_not_found(backtesting_dispatcher):
...
async def test_open_orders(backtesting_dispatcher):
    e = exchange.Exchange(
        backtesting_dispatcher,
        {
            "ARS": Decimal("-2000.05"),
...
async def test_cancel_inexistent_order(backtesting_dispatcher):
    e = exchange.Exchange(
        backtesting_dispatcher,
        {
            "ARS": Decimal("-2000.05"),
...
async def test_order_requests(order_plan, immediate_order_processing, backtesting_dispatcher):
    e = exchange.Exchange(
        backtesting_dispatcher,
        {"USD": Decimal("1e6")},
...
async def test_invalid_requests(order_request, backtesting_dispatcher):
    e = exchange.Exchange(
        backtesting_dispatcher,
        {
            "USD": Decimal("1e6"),
...
async def test_not_enough_balance(order_request, backtesting_dispatcher):
    e = exchange.Exchange(
        backtesting_dispatcher,
        {
            "USD": Decimal("1e3"),
...
async def test_small_fill_is_ignored_after_rounding(backtesting_dispatcher):
...
async def test_liquidity_is_exhausted_and_order_is_canceled(backtesting_dispatcher):
    e = exchange.Exchange(
        backtesting_dispatcher,
        {
            "USD": Decimal("1e12")
...
async def test_balance_is_on_hold_while_order_is_open(backtesting_dispatcher):
    e = exchange.Exchange(
        backtesting_dispatcher,
        {
            "USD": Decimal(1000)
