...
* A **position manager** that is responsible for executing trades in response to trading signals generated by the strategy.
  The examples include a `backtesting position manager <https://github.com/gbeced/basana/blob/feature/loans/samples/backtesting/position_manager.py>`_
  and a `Binance position manager <https://github.com/gbeced/basana/blob/feature/loans/samples/binance/position_manager.py>`_.
